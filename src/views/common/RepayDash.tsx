...
import { useUserAssetBalance, useUserReserveAssetBalancesDaiWei, useUserVariableDebtTokenBalances } from "../../queries/userAssets";
...
export const RepayDash: React.FC<RepayDashProps> = ({
  token,
}) => {
  // General
    ...
    <VStack spacing="0" w="100%" bg="primary.900" rounded="lg">
      ...
      >
        <Flex w="30%" spacing={spacings.md} mr={{ base: "0rem", md: "1rem" }} alignItems={{base:"flex-start", lg:"center"}}  justifyContent="flex-start" flexDirection={{base:"column", lg:"row"}}>
          <Text fontSize={{ base: fontSizes.sm, md: fontSizes.md }}  pr="1rem">You Borrowed</Text>
	  ...
	  justifyContent="space-between">
        ...
        <Stack justifyContent="flex-start" mr={{ base: "0.7rem", md: "1rem" }}>
          <Text fontSize={{ base: fontSizes.sm, md: fontSizes.md }} >{isSmallerThan900 ? "Collateral" : "Your collateral"}</Text>
        ...
        <VStack justifyContent="flex-start" mr={{ base: "0.7rem", md: "1rem" }}>
          <Text fontSize={{ base: fontSizes.sm, md: fontSizes.md }} >{isSmallerThan900 ? "Composition" : "Collateral composition"}</Text>
<Popover trigger='hover' >
       ...
       <Stack justifyContent="flex-start" mr={{ base: "0.2rem", md: "1rem" }} whiteSpace="nowrap">
          <HStack pr={{ base: "0rem", md: "1rem" }}>
            <Text fontSize={{ base: fontSizes.sm, md: fontSizes.md }}>{isSmallerThan900 ? "LTV" : "Loan to value"	}</Text>
          </HStack>
		  <HStack pr={{ base: "0rem", md: "1rem" }} align="center">
          <Text fontSize={{ base: fontSizes.md, md: fontSizes.lg, lg:fontSizes.xl }} fontWeight="bold" >
            {currentLtv ? (currentLtv.toUnsafeFloat() * 100).toLocaleString() : "-"} %
          </Text>
		  <ModalIcon position="relative" top="0" right="0" onOpen={() => {}} />
		  </HStack>
        </Stack>
